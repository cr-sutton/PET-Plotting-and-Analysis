...
data_path = 'cPET3D_tracer_preacid_nan'
...
C0_Sim=np.linspace(1, 30, num=40)
#1-25 seems to be the best fit
Df_Sim = np.logspace(-7, -11, num=50) # in cm^2/s
#-6 to -10 is a good fit
#10-7 to 10-8 according to Gupta et al 2020
t_arrival = np.linspace(10, 1000, num=100) # in cm^2/s
...
color = iter(cm.rainbow(np.linspace(0, 1, len(binary_frac_matrix>0))))
    ...
    for c in range(0,ncol):
        ...
        for s in range(0, 1):
            ...
            if binary_frac_matrix[r,c,s]==1:     
                ...
                c = next(color)
                plt.plot(t_PETSim, btcArray, linestyle='solid', linewidth=1, alpha = 1, c=c)
                M1d_sim = diffusion_integration_fun_v2(btcArray, dx/2, t_PETSim, dfit, cfit)
                plt.plot(t_PETSim+tfit, M1d_sim*2, linestyle='dashed', linewidth=1, alpha = 1, c=c)
                n+1       
...
plot_2d(rmse_sim[:,:,20], 1, 1, 'rmse', 'viridis')
